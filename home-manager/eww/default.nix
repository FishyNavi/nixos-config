{pkgs,...}:
let
  cava = pkgs.writeShellScriptBin "eww-cava" ''
    # Nix will replace ${pkgs.cava} with the actual store path
    ...
    ascii_max_range = 7
    EOF
    ) | ${pkgs.gnused}/bin/sed -u 's/;//g;s/0/⠀/g;s/1/▂/g;s/2/▃/g;s/3/▄/g;s/4/▅/g;s/5/▆/g;s/6/▇/g;s/7/█/g'
  '';
  album_art = pkgs.writeShellScriptBin "eww-album-art" ''
    #!/bin/bash
