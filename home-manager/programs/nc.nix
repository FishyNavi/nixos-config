{pkgs, ... }:
let 
    script = pkgs.writeShellScriptBin "rofi-dunst-manager" ''
      #!/usr/bin/env bash
      # credit to https://github.com/theopn for this awesome script!!
      ...
      function get_notif_list() {
        ...
        echo "$HIST_JSON" | "${pkgs.jq}/bin/jq" -r --arg uptime "$uptime" '
        .data[0][] |
      ...
      function set_shell_var() {
        echo "$HIST_JSON" | "${pkgs.jq}/bin/jq" -r --argjson idx "$1" '
        .data[0][$idx] |
